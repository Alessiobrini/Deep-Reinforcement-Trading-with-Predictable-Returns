...
class PPOActorCritic(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(hidden_units_value)):
            if i == 0:
                ...
                critic_modules.append(nn.Linear(hidden_units_value[i - 1], hidden_units_value[i]))
                if activation == 'relu':
                    critic_modules.append(nn.ReLU())
                elif activation == 'tanh':
                    critic_modules.append(nn.Tanh())
        ...
        if batch_norm_value_out:
            critic_modules = critic_modules + [nn.Linear(hidden_units_value[-1], 1), nn.BatchNorm1d(1, affine=False)]
        else:
        ...
        for i in range(len(hidden_units_actor)):
            if i == 0:
                ...
                actor_modules.append(nn.Linear(hidden_units_actor[i - 1], hidden_units_actor[i]))
                if activation == 'relu':
                    actor_modules.append(nn.ReLU())
                elif activation == 'tanh':
                    actor_modules.append(nn.Tanh())
    ...
    def forward(self, x):
        ...
        if self.policy_type == 'continuous':
            ...
            mu    = self.actor(x)
            std   = self.log_std.exp().expand_as(mu) # make the tensor of the same size of mu
            dist  = Normal(mu, std)
            ...
            if torch.isnan(logits).sum()>0:
                logits = torch.empty(logits.shape).uniform_(-0.01,0.01).type(torch.FloatTensor)
    ...
    def init_weights(self):
        ...
        nn.init.normal_(self.actor[-1].weight, mean=0., std=0.01)
...
class PPO:
    ...
    def ppo_iter(self):
        ...
        for _ in range(len_rollout // self.batch_size):
            ...
            yield (torch.from_numpy(states[rand_ids, :]).float().to(self.device),
