...
class PER_buffer():
    ...
    def __init__(self, PER_e,PER_a,PER_b,final_PER_b,PER_b_steps,PER_b_growth,
                 final_PER_a,PER_a_steps,PER_a_growth,max_experiences):
        ...
        self.experience = {'s': np.zeros(max_experiences, dtype=object), 
    ...
    def add(self, exp):
        ...
        if self.data_pointer >= self.max_experiences:  # If we're above the capacity, we go back to first index (we overwrite)
            self.data_pointer = 0
    ...
    def get_leaf(self, v):
        ...
        self.exp_leaf = {key:value[exp_index] for key, value in self.experience.items()}
