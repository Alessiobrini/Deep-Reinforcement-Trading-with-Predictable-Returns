...
def PreTraining(
    ...
    for i in tqdm(iterable=range(N_pretrain + 1), desc="PreTraining DQNetwork"):
        ...
        if (
            save_ckpt_pretrained_model
            and (i % save_ckpt_pretrained_steps == 0)
            and (i > 0)
        ):
            PreTrainQNet.model.save_weights(
                os.path.join(
                    savedpath, "ckpt_pt", "DQN_{}_it_pretrained_weights".format(i)
                ),
                save_format="tf",
            )
    pdb.set_trace()
    PreTrainQNet.model.save_weights(
        os.path.join(savedpath, "DQN_pretrained_weights"), save_format="tf"
    )
