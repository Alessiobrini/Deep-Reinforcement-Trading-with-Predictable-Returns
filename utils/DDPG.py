...
class CriticNetwork(tf.keras.Model):
    ...
    def __init__(self, 
        ...
        if self.bnflag_input:
            self.bnorm_layer = BatchNormalization(center=False, 
                                                  scale=False)
...
class DDPG:
    ...
    def __init__(self, 
        ...
        if DDPG_type == 'DDPG':
            if optimizer_name == 'sgd':
                ...
                self.optimizer_Q = AdamW(weight_decay = weight_decay_Q,learning_rate=lr_Q, beta_1=beta_1, beta_2=beta_2, 
                                                          epsilon=eps_opt, amsgrad=False)
            ...
            if optimizer_name == 'sgd':
                self.optimizer_Q1 = tf.keras.optimizers.SGD(learning_rate=lr_Q, momentum=0.0, 
                                                         nesterov=False)
                self.optimizer_Q2 = tf.keras.optimizers.SGD(learning_rate=lr_Q, momentum=0.0, 
                                                         nesterov=False)
                self.optimizer_p = tf.keras.optimizers.SGD(learning_rate=lr_p, momentum=0.0, 
                                                         nesterov=False)
            elif optimizer_name == 'adam':
                self.optimizer_Q1 = tf.keras.optimizers.Adam(learning_rate=lr_Q, beta_1=beta_1, beta_2=beta_2, 
                                                          epsilon=eps_opt, amsgrad=False)
                self.optimizer_Q2 = tf.keras.optimizers.Adam(learning_rate=lr_Q, beta_1=beta_1, beta_2=beta_2, 
                                                          epsilon=eps_opt, amsgrad=False)
                self.optimizer_p = tf.keras.optimizers.Adam(learning_rate=lr_p, beta_1=beta_1, beta_2=beta_2, 
                                                      epsilon=eps_opt, amsgrad=False)
            elif optimizer_name == 'rmsprop':
                self.optimizer_Q1 = tf.keras.optimizers.RMSprop(learning_rate=lr_Q, rho=beta_1, momentum=0.0, epsilon=eps_opt,
                                                             centered=False)
                self.optimizer_Q2 = tf.keras.optimizers.RMSprop(learning_rate=lr_Q, rho=beta_1, momentum=0.0, epsilon=eps_opt,
                                                             centered=False)
                self.optimizer_p = tf.keras.optimizers.RMSprop(learning_rate=lr_p, rho=beta_1, momentum=0.0, epsilon=eps_opt,
                                                              centered=False)
            elif optimizer_name == 'adamw':
                self.optimizer_Q1 = AdamW(weight_decay = weight_decay_Q,learning_rate=lr_Q, beta_1=beta_1, beta_2=beta_2, 
                                                          epsilon=eps_opt, amsgrad=False)
                self.optimizer_Q2 = AdamW(weight_decay = weight_decay_Q,learning_rate=lr_Q, beta_1=beta_1, beta_2=beta_2, 
                                                          epsilon=eps_opt, amsgrad=False)
                self.optimizer_p = AdamW(weight_decay = weight_decay_p, learning_rate=lr_p, beta_1=beta_1, beta_2=beta_2, 
                                                      epsilon=eps_opt, amsgrad=False)
            else:
                print('Choose an available optimizer!')
                sys.exit()
        ...
        if DDPG_type == 'DDPG':
            if recurrent_env:
                self.Q_model = CriticRecurrentNetwork(seed, num_states, hidden_memory_units, hidden_units_Q, num_actions, 
                                               batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
                                               output_init, delayed_actions, 
                                               modelname='Q'+ modelname)   
                self.p_model = ActorRecurrentNetwork(seed, num_states, hidden_memory_units, hidden_units_p, num_actions,
                                               batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
                                               output_init, modelname='p'+ modelname) 
            else:
                self.Q_model = CriticNetwork(seed, num_states, hidden_units_Q, num_actions, 
                                               batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
                                               output_init, delayed_actions, 
                                               modelname='Q'+ modelname)   
                self.p_model = ActorNetwork(seed, num_states, hidden_units_p, num_actions,
                                               batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
                                               output_init, modelname='p'+ modelname) 
        elif DDPG_type == 'TD3':
            if recurrent_env:
                self.Q1_model = CriticRecurrentNetwork(seed, num_states, hidden_memory_units, hidden_units_Q, num_actions, 
                                               batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
                                               output_init, delayed_actions, 
                                               modelname='Q'+ modelname)   
                self.Q2_model = CriticRecurrentNetwork(seed, num_states, hidden_memory_units, hidden_units_Q, num_actions, 
                                               batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
                                               output_init, delayed_actions, 
                                               modelname='Q'+ modelname)  
                self.p_model = ActorRecurrentNetwork(seed, num_states, hidden_memory_units, hidden_units_p, num_actions,
                ...
                self.Q1_model = CriticNetwork(seed, num_states, hidden_units_Q, num_actions, 
                                               batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
                                               output_init, delayed_actions, 
                                               modelname='Q'+ modelname)   
                self.Q2_model = CriticNetwork(seed, num_states, hidden_units_Q, num_actions, 
                                               batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
                                               output_init, delayed_actions, 
                                               modelname='Q'+ modelname)   
                self.p_model = ActorNetwork(seed, num_states, hidden_units_p, num_actions,
        ...
        if noise == 'G':
            self.action_noise = GaussianActionNoise(mu=np.ones(nb_actions)*mu, 
                                                             sigma=float(stddev_noise) * np.ones(nb_actions),
                                                             rng = self.rng)
        elif noise == 'OU':
            self.action_noise = OrnsteinUhlenbeckActionNoise(mu=np.ones(nb_actions)*mu, 
                                                             sigma=float(stddev_noise) * np.ones(nb_actions),
                                                             theta=theta,
                                                             rng=self.rng)
            
        if DDPG_type == 'TD3':
            self.policy_noise = GaussianActionNoise(mu=np.ones(nb_actions)*mu, 
    ...
    def train(self, TargetNet, iteration, env=None):
        ...
        with tf.GradientTape(persistent=True) as tape1:
            ...
            if self.DDPG_type == 'TD3':
                current_q1 = self.Q1_model(states.astype('float32'), actions.astype('float32'),
                                         store_intermediate_outputs=True)
                current_q2 = self.Q2_model(states.astype('float32'), actions.astype('float32'),
                                         store_intermediate_outputs=True)

                noise = tf.cast(tf.clip_by_value(self.policy_noise(), -self.noise_clip,self.noise_clip),
                                tf.float32)

                action_next = tf.clip_by_value(TargetNet.p_model(states_next.astype('float32')) + noise,
                                               - self.action_limit, self.action_limit)
                
                target_q1 = TargetNet.Q1_model(states_next.astype('float32'), action_next)
                target_q2 = TargetNet.Q2_model(states_next.astype('float32'), action_next)
                ...
                action_next = TargetNet.p_model(states_next.astype('float32')) #TODO add noise here
        ...
        if self.DDPG_type == 'TD3':
            ...
            if self.clipgrad == 'norm':
                gradients_q1 = [(tf.clip_by_norm(gv, self.clipnorm)) for gv in gradients_q1]
                gradients_q2 = [(tf.clip_by_norm(gv, self.clipnorm)) for gv in gradients_q2]
            elif self.clipgrad == 'value':
                gradients_q1 = [(tf.clip_by_value(gv,-self.clipvalue, self.clipvalue)) for gv in gradients_q1]
                gradients_q2 = [(tf.clip_by_value(gv,-self.clipvalue, self.clipvalue)) for gv in gradients_q2]
                # TODO no glob norm implemented
        ...
        with tf.GradientTape() as tape2:
            if self.DDPG_type == 'TD3':
                current_q_pg = self.Q1_model(states.astype('float32'), self.p_model(states.astype('float32'),
                                                                                   store_intermediate_outputs=True))
            else:
    ...
    def uniform_action(self):
        ...
        return self.rng.uniform(-1.0, 1.0,self.nb_actions)
