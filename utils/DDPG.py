...
class DDPG:
    ...
    def __init__(self, 
        ...
        #     self.optimizer_Q = AdamW(weight_decay = weight_decay,learning_rate=lr_Q, beta_1=beta_1, beta_2=beta_2, 
        #                                               epsilon=eps_opt, amsgrad=False)
        ...
        if recurrent_env:
            self.Q_model = CriticRecurrentNetwork(seed, num_states, hidden_memory_units, hidden_units, num_actions, 
                                           batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
                                           output_init, delayed_actions, 
                                           modelname='Q'+ modelname)   
            self.p_model = ActorRecurrentNetwork(seed, num_states, hidden_memory_units, hidden_units, num_actions,
                                           batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
            ...
            self.Q_model = CriticNetwork(seed, num_states, hidden_units, num_actions, 
                                           ...
                                           output_init, delayed_actions, 
                                           modelname='Q'+ modelname)   
            self.p_model = ActorNetwork(seed, num_states, hidden_units, num_actions,
                                           batch_norm_input, batch_norm_hidden, activation, kernel_initializer, 
                                           output_init, modelname='p'+ modelname) 
    ...
    def copy_weights(self, TrainNet):
        ...
        for v1q, v2q in zip(variables1q, variables2q):
            vsoftq = (1 - self.tau) * v1q + self.tau * v2q
        ...
        for v1p, v2p in zip(variables1p, variables2p):
            vsoftp = (1 - self.tau) * v1p + self.tau * v2p
