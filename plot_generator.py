...
def runplot_value(p):
    ...
    latest_subdir = max(all_subdirs, key=os.path.getmtime)
    length = os.path.split(latest_subdir)[-1]
    data_dir = "outputs/{}/{}/{}/{}".format(outputClass, outputModel, length, experiment)
...
def runplot_holding(p):
    ...
    for i,model in enumerate(outputModel):
        ...
        latest_subdir = max(all_subdirs, key=os.path.getmtime)
        length = os.path.split(latest_subdir)[-1]
        experiment = [exp for exp in os.listdir("outputs/{}/{}/{}".format(outputClass, model, length)) if seed in exp][0]
        ...
        if 'DQN' in tag:
            ...
            train_agent.model =  load_DQNmodel(data_dir, query('%N_TRAIN'), model=train_agent.model)
        ...
        axes[i].set_title('_'.join(['mv_res',split[-1]]).replace('_',' '), fontsize=10)
        axes2[i].set_title('_'.join(['mv_res',split[-1]]).replace('_',' '), fontsize=10)
