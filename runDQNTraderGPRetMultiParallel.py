...
Param = readConfigYaml(os.path.join(os.getcwd(),'config','paramDQNGPRetMultiParallel.yaml'))
logging.info('Successfully read config file with parameters...')

var1 = Param[Param['varying_par']]
var2 = Param[Param['varying_par2']]

variables = []

for xs in itertools.product(var1,var2):
    variables.append(xs)

num_cores = len(variables)


def RunExp(p1,p2,Param):
    
    
    Param[Param['varying_par']] = p1
    Param[Param['varying_par2']] = p2
...
if __name__ == "__main__":
    Parallel(n_jobs=num_cores)(delayed(RunExp)(p[0],p[1],Param) for p in variables)